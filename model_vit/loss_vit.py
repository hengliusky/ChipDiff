...
class Loss_vit(torch.nn.Module):
    ...
    def forward(self, outputs, source,out_prev=None,use_dir=True,target=None,frac_cont=1.0):
        ...
        if self.lambdas['lambda_global_ssim'] > 0:
            ...
            gobal_ssim_loss = losses['loss_global_ssim'] * self.lambdas['lambda_global_ssim']
            print('vit content loss: ', gobal_ssim_loss)
            #loss_G += gobal_ssim_loss
        ...
        if self.lambdas['lambda_contra_ssim'] > 0:
            ...
            print('patch contrastive loss: ',losses['loss_contra_ssim'] * self.lambdas['lambda_contra_ssim']*frac_cont)
            losses_val['loss_contra_ssim'] = losses['loss_contra_ssim'].item()

        if use_dir:
            #self.lambdas['lambda_trg'] = 0.0
            if self.lambdas['lambda_dir_cls'] > 0:
                ...
                #print('sem: ', losses['loss_dir_cls'] * self.lambdas['lambda_dir_cls'])
        ...
        if target is not None:
            if self.lambdas['lambda_trg'] > 0:
                ...
                cls_style_loss = losses['loss_trg'] * self.lambdas['lambda_trg']
                print('vit style loss: ', cls_style_loss)
                #loss_G += cls_style_loss
                losses_val['loss_trg'] = losses['loss_trg'].item()

        return loss_G, losses_val, gobal_ssim_loss, cls_style_loss
